/*
  # Fix New Team Signup - Allow Pending Team Setup

  ## Problem
  When a user signs up with an invite code that has team_id=NULL (for creating a new team),
  the signup fails if they haven't provided a team name yet. This is because the current
  trigger requires EITHER a team_id OR a new_team_name, but the UI flow collects the team
  name during onboarding, not during signup.

  ## Solution
  Add SCENARIO 3: Create user without team assignment when:
  - Invite code has no team_id (new team invite)
  - User hasn't provided a team name yet (pending_team_setup flag)
  
  The user will complete team creation during onboarding via the separate RPC function.

  ## Changes
  - Modified handle_new_user_signup() to allow pending team setup
  - Users with pending_team_setup=true will have team_id=NULL initially
*/
...
BEGIN
  ...
  BEGIN
    ...
    is_pending_team_setup := COALESCE((NEW.raw_user_meta_data->>'pending_team_setup')::boolean, false);

    RAISE NOTICE 'Extracted - invite_code: %, new_team_name: %, pending_team_setup: %', 
                 user_invite_code, new_team_name, is_pending_team_setup;
    ...
    IF user_invite_code IS NOT NULL THEN
      ...
      RAISE NOTICE 'Invite details - team_id: %, role: %, view_financial: %', 
                   invite_team_id, invite_role, invite_view_financial;

      -- SCENARIO 1: Creating a new team with team name provided
      ...
      -- SCENARIO 3: New team invite but team name will be provided during onboarding
      ELSIF invite_team_id IS NULL AND is_pending_team_setup THEN
        RAISE NOTICE 'SCENARIO 3: Pending team setup - will complete during onboarding';

        -- Create public.users record WITHOUT team assignment (will be set during onboarding)
        BEGIN
          INSERT INTO public.users (id, email, name, team_id, role, view_financial, created_at, updated_at)
          VALUES (
            NEW.id,
          ...
          RAISE NOTICE 'SUCCESS: Created public.users record with pending team setup';
        EXCEPTION WHEN OTHERS THEN
          GET STACKED DIAGNOSTICS 
            error_message = MESSAGE_TEXT,
            error_detail = PG_EXCEPTION_DETAIL,
      ...
      -- SCENARIO 4: Invalid invite
      ELSE
        RAISE EXCEPTION 'Invalid signup: invite has team_id=% and new_team_name=% and pending_team_setup=%', 
                        invite_team_id, new_team_name, is_pending_team_setup;
      END IF;
    ELSE
