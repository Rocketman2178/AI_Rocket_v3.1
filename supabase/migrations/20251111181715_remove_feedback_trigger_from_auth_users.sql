/*
  # Remove feedback initialization trigger from auth.users

  1. Problem
    - Trigger on_user_created_init_feedback tries to insert into user_feedback_status
    - user_feedback_status has foreign key to public.users
    - public.users record doesn't exist yet (we removed the signup trigger)
    - Foreign key constraint violation causes signup to fail
    
  2. Solution
    - Remove the trigger from auth.users
    - Initialize user_feedback_status in the complete_user_signup RPC function instead
    - This ensures proper ordering: auth.users → public.users → user_feedback_status
    
  3. Changes
    - Drop the trigger from auth.users
    - Update complete_user_signup() to also initialize feedback status
*/

-- Drop the feedback initialization trigger
DROP TRIGGER IF EXISTS on_user_created_init_feedback ON auth.users;

-- Update the complete_user_signup function to also initialize feedback status
...
BEGIN
  ...
  IF p_invite_code IS NOT NULL THEN
    ...
    IF invite_team_id IS NULL THEN
      ...
      -- Initialize feedback status (NEW)
